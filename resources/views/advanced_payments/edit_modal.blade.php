            ...
            <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                ...
                <div class="row">
                    <div class="form-group col-lg-12 mb-5 mt-5">
                        ...
                        {{ Form::select('patient_id', $patients ?? [], null, ['class' => 'border border-2 bg-white py-2 form-select form-select-solid fw-bold', 'id' => 'editPatientId','placeholder' => 'Select Patient', 'required','data-control' => 'select2']) }}
                    </div>
                    <div class="form-group col-lg-12 mb-5 mt-5">
                        ...
                        {{ Form::text('receipt_no', null, ['class' => 'border border-2 bg-white py-2 form-control form-control-solid','id'=> 'editReceiptNo','required','readonly']) }}
                    </div>
                    <div class="form-group col-lg-12 mb-5 mt-5">
                        {{ Form::label('amount', __('messages.advanced_payment.amount').(':'),['class' => 'form-label required fs-6 fw-bolder text-gray-700 mb-1']) }}
                        {{ Form::text('amount', null, ['class' => 'border border-2 bg-white py-2 form-control price-input form-control-solid','id'=> 'editAmount','required', 'onkeyup' => 'if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,"")', 'maxlength' => '7']) }}
                    </div>
                    <div class="form-group col-lg-12 mb-5 mt-5">
                        {{ Form::label('date', __('messages.advanced_payment.date').(':'),['class' => 'form-label required fs-6 fw-bolder text-gray-700 mb-1']) }}
                        {{ Form::text('date', null, ['class' => 'border border-2 bg-white py-2 form-control form-control-solid','id' => 'editDate','required','autocomplete' => 'off']) }}
                    </div>
                </div>
                <div class="text-right mt-5">
                    {{ Form::button(__('messages.common.save'), ['type' => 'submit','class' => 'btn btn-primary me-2 border border-2 py-2 ','id' => 'btnEditSave','data-loading-text' => "<span class='spinner-border spinner-border-sm'></span> Processing..."]) }}
