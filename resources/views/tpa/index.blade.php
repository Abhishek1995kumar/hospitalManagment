...
@section('content')
    ...
    <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">TPA Records</h1>
            </div>
        <div class="card-header border-0 pt-6">
            <!--State code serch-->
<div>
        <input type="search" id="tpamasterss" data-datatable-filter="search" name="search" 
...
<!--end code serch -->
            <div class="card-toolbar">
                <div class="d-flex align-items-center py-1">
                            ...
                            <div class="px-7 py-5">
                                ...
                                <div class="d-flex justify-content-end">
                                    <button type="reset" class="btn btn-sm fs-6 btn-light btn-active-light-primary me-2"
                                            id="resetFilter"
                                            data-kt-menu-dismiss="true">{{ __('messages.common.reset') }}
                                        </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div
                        ...
                        <a href="{{ route('tpa.create') }}"
                               class="menu-link px-5">{{ __('Add Tpamaster') }}</a>
                        </div>
                        <div class="menu-item px-5">
        ...
        <div class="card-body pt-0 fs-6 py-8 px-8  px-lg-10 text-gray-700">
                            ...
                            <table class="table table-responsive-sm align-middle table-row-dashed fs-6 gy-5 dataTable no-footer w-100" id="tt">
                                <thead>
                                ...
                                <tbody class="text-gray-600 fw-bold" id="tpamast">
                                    @foreach($tpas as $item)
                                    <tr>
                                    <td>{{ $item->id  }}</td>
                                        <td>{{ $item->paymentmode  }}</td>
                                        <td>{{ $item->tpapaymenttype }}</td>
                                       
                                        <td>
                                        <a href="{{url('/tpamaster/delete/')}}/{{$item->id}}">
...
<!--script for serch box-->
   <script type="text/javascript">
    $(document).ready(function(){
        $("#tpamasterss").on("keyup",function(){
           var value = $(this).val().toLowerCase();
           //alert(value);
           $("#tpamast tr").filter(function(){
