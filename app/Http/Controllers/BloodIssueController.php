...
class BloodIssueController extends AppBaseController
{
    ...
    public function store(BloodIssueRequest $request)
    {
        try {
            ...
            $data = $this->bloodIssueRepository->create($input);
            $last_insert_id = DB::getPdo()->lastInsertId();
            $blood_group_id = DB::select("SELECT blood_group FROM blood_issues WHERE id = $last_insert_id");
            if($blood_group_id[0]->blood_group > 0){
                $blood_id = $blood_group_id[0]->blood_group;
                DB::table('blood_banks')
                    ->where('id', $blood_id)  // find your user by their email
                    ->update(array('remained_bags' => DB::raw('remained_bags - 1')));
            }
