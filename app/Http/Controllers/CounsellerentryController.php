...
class CounsellerentryController extends Controller
{
    ...
    function create (){
        // $paymode= Tpa::all();
        $doctorref = User::where('department_id', '=', '2')->get();  // echo "<pre>";// print_r($doctorref);// die();
        $doctoranalysispatient = DB::table('newdoctoranalysis')->get();  // echo "<pre>";// print_r($doctoranalysispatient);// die();
        $surcateg = DB::table('subcategory')->get();  // echo "<pre>"; // print_r($surcateg);   // isse hamra paise ka data aa rha hai // die();
        $refinjection = DB::table('product')->get();  // echo "<pre>"; // print_r($refinjection); // die();
        $tpapaytype=DB::table('tpamaster')->get();
        $paymode = DB::table('tpapaymentmode') // echo "<pre>";  print_r($paymode); die();
        ...
        ->get();  // echo "<pre>"; // print_r($paymode); // die();
        return view('counseller.create',compact('doctoranalysispatient','doctorref','surcateg','refinjection','paymode', 'tpapaytype'));
    }
